...
const btn3_cfg_path = path.join(path.dirname(__dirname), "config-site", "button-3", "btn-config.cfg")
const btn4_cfg_path = path.join(path.dirname(__dirname), "config-site", "button-4", "btn-config.cfg")
const btn5_cfg_path = path.join(path.dirname(__dirname), "config-site", "button-5", "btn-config.cfg")
const btn6_cfg_path = path.join(path.dirname(__dirname), "config-site", "button-6", "btn-config.cfg")
const btn7_cfg_path = path.join(path.dirname(__dirname), "config-site", "button-7", "btn-config.cfg")
...
function update_values() {
    ...
    */
    ...
    btn1_cfg = fs.readFileSync(btn1_cfg_path, "utf-8")
    btn2_cfg = fs.readFileSync(btn2_cfg_path, "utf-8")
    btn3_cfg = fs.readFileSync(btn3_cfg_path, "utf-8")
    btn4_cfg = fs.readFileSync(btn4_cfg_path, "utf-8")
    btn5_cfg = fs.readFileSync(btn5_cfg_path, "utf-8")
    btn6_cfg = fs.readFileSync(btn6_cfg_path, "utf-8")
    btn7_cfg = fs.readFileSync(btn7_cfg_path, "utf-8")
    btn8_cfg = fs.readFileSync(btn8_cfg_path, "utf-8")
    ...
    serialPort.on('data', function (data) {
        ...
        if (data_str.indexOf("-") == -1) {
            ...
            for (line_index in data_array_newline) {
                ...
                if      (data_array[0] == "name1") { btn1_name = data_array[1]; fs.writeFileSync(btn1_cfg_path, btn1_name + "\n") }
                else if (data_array[0] == "name2") { btn2_name = data_array[1]; fs.writeFileSync(btn2_cfg_path, btn2_name + "\n") }
                else if (data_array[0] == "name3") { btn3_name = data_array[1]; fs.writeFileSync(btn3_cfg_path, btn3_name + "\n") }
                else if (data_array[0] == "name4") { btn4_name = data_array[1]; fs.writeFileSync(btn4_cfg_path, btn4_name + "\n") }
                else if (data_array[0] == "name5") { btn5_name = data_array[1]; fs.writeFileSync(btn5_cfg_path, btn5_name + "\n") }  
                else if (data_array[0] == "name6") { btn6_name = data_array[1]; fs.writeFileSync(btn6_cfg_path, btn6_name + "\n") }
                else if (data_array[0] == "name7") { btn7_name = data_array[1]; fs.writeFileSync(btn7_cfg_path, btn7_name + "\n") }
                else if (data_array[0] == "name8") { btn8_name = data_array[1]; fs.writeFileSync(btn8_cfg_path, btn8_name + "\n") }

                else if (data_array[0] == "led1base") { values = data_array[1].toString(); led1base_rgb = values.split("/"); led1base_hex = rgbToHex(parseInt(led1base_rgb[0]), parseInt(led1base_rgb[1]), parseInt(led1base_rgb[2])); fs.appendFileSync(btn1_cfg_path, led1base_hex + "\n") }
                else if (data_array[0] == "led2base") { values = data_array[1].toString(); led2base_rgb = values.split("/"); led2base_hex = rgbToHex(parseInt(led2base_rgb[0]), parseInt(led2base_rgb[1]), parseInt(led2base_rgb[2])); fs.appendFileSync(btn2_cfg_path, led2base_hex + "\n") }
                else if (data_array[0] == "led3base") { values = data_array[1].toString(); led3base_rgb = values.split("/"); led3base_hex = rgbToHex(parseInt(led3base_rgb[0]), parseInt(led3base_rgb[1]), parseInt(led3base_rgb[2])); fs.appendFileSync(btn3_cfg_path, led3base_hex + "\n") }
                else if (data_array[0] == "led4base") { values = data_array[1].toString(); led4base_rgb = values.split("/"); led4base_hex = rgbToHex(parseInt(led4base_rgb[0]), parseInt(led4base_rgb[1]), parseInt(led4base_rgb[2])); fs.appendFileSync(btn4_cfg_path, led4base_hex + "\n") }
                else if (data_array[0] == "led5base") { values = data_array[1].toString(); led5base_rgb = values.split("/"); led5base_hex = rgbToHex(parseInt(led5base_rgb[0]), parseInt(led5base_rgb[1]), parseInt(led5base_rgb[2])); fs.appendFileSync(btn5_cfg_path, led5base_hex + "\n") }
                else if (data_array[0] == "led6base") { values = data_array[1].toString(); led6base_rgb = values.split("/"); led6base_hex = rgbToHex(parseInt(led6base_rgb[0]), parseInt(led6base_rgb[1]), parseInt(led6base_rgb[2])); fs.appendFileSync(btn6_cfg_path, led6base_hex + "\n") }
                else if (data_array[0] == "led7base") { values = data_array[1].toString(); led7base_rgb = values.split("/"); led7base_hex = rgbToHex(parseInt(led7base_rgb[0]), parseInt(led7base_rgb[1]), parseInt(led7base_rgb[2])); fs.appendFileSync(btn7_cfg_path, led7base_hex + "\n") }
                else if (data_array[0] == "led8base") { values = data_array[1].toString(); led8base_rgb = values.split("/"); led8base_hex = rgbToHex(parseInt(led8base_rgb[0]), parseInt(led8base_rgb[1]), parseInt(led8base_rgb[2])); fs.appendFileSync(btn8_cfg_path, led8base_hex + "\n") }

                else if (data_array[0] == "led1fade") { values = data_array[1].toString(); led1fade_rgb = values.split("/"); led1fade_hex = rgbToHex(parseInt(led1fade_rgb[0]), parseInt(led1fade_rgb[1]), parseInt(led1fade_rgb[2])); fs.appendFileSync(btn1_cfg_path, led1fade_hex + "\n") }
                else if (data_array[0] == "led2fade") { values = data_array[1].toString(); led2fade_rgb = values.split("/"); led2fade_hex = rgbToHex(parseInt(led2fade_rgb[0]), parseInt(led2fade_rgb[1]), parseInt(led2fade_rgb[2])); fs.appendFileSync(btn2_cfg_path, led2fade_hex + "\n") }
                else if (data_array[0] == "led3fade") { values = data_array[1].toString(); led3fade_rgb = values.split("/"); led3fade_hex = rgbToHex(parseInt(led3fade_rgb[0]), parseInt(led3fade_rgb[1]), parseInt(led3fade_rgb[2])); fs.appendFileSync(btn3_cfg_path, led3fade_hex + "\n") }
                else if (data_array[0] == "led4fade") { values = data_array[1].toString(); led4fade_rgb = values.split("/"); led4fade_hex = rgbToHex(parseInt(led4fade_rgb[0]), parseInt(led4fade_rgb[1]), parseInt(led4fade_rgb[2])); fs.appendFileSync(btn4_cfg_path, led4fade_hex + "\n") }
                else if (data_array[0] == "led5fade") { values = data_array[1].toString(); led5fade_rgb = values.split("/"); led5fade_hex = rgbToHex(parseInt(led5fade_rgb[0]), parseInt(led5fade_rgb[1]), parseInt(led5fade_rgb[2])); fs.appendFileSync(btn5_cfg_path, led5fade_hex + "\n") }
                else if (data_array[0] == "led6fade") { values = data_array[1].toString(); led6fade_rgb = values.split("/"); led6fade_hex = rgbToHex(parseInt(led6fade_rgb[0]), parseInt(led6fade_rgb[1]), parseInt(led6fade_rgb[2])); fs.appendFileSync(btn6_cfg_path, led6fade_hex + "\n") }
                else if (data_array[0] == "led7fade") { values = data_array[1].toString(); led7fade_rgb = values.split("/"); led7fade_hex = rgbToHex(parseInt(led7fade_rgb[0]), parseInt(led7fade_rgb[1]), parseInt(led7fade_rgb[2])); fs.appendFileSync(btn7_cfg_path, led7fade_hex + "\n") }
                else if (data_array[0] == "led8fade") { values = data_array[1].toString(); led8fade_rgb = values.split("/"); led8fade_hex = rgbToHex(parseInt(led8fade_rgb[0]), parseInt(led8fade_rgb[1]), parseInt(led8fade_rgb[2])); fs.appendFileSync(btn8_cfg_path, led8fade_hex + "\n") }
                    ...
                    for (key in btn1_keys) {
                        if (key != btn1_keys.length - 1) {
                            fs.appendFileSync(btn1_cfg_path, btn1_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn1_cfg_path, btn1_keys[key])
                        }
                    }
                    ...
                    for (key in btn2_keys) {
                        if (key != btn2_keys.length - 1) {
                            fs.appendFileSync(btn2_cfg_path, btn2_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn2_cfg_path, btn2_keys[key])
                        }
                    }
                    ...
                    for (key in btn3_keys) {
                        if (key != btn3_keys.length - 1) {
                            fs.appendFileSync(btn3_cfg_path, btn3_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn3_cfg_path, btn3_keys[key])
                        }
                    }
                    ...
                    for (key in btn4_keys) {
                        if (key != btn4_keys.length - 1) {
                            fs.appendFileSync(btn4_cfg_path, btn4_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn4_cfg_path, btn4_keys[key])
                        }
                }
                    ...
                    for (key in btn5_keys) {
                        if (key != btn5_keys.length - 1) {
                            fs.appendFileSync(btn5_cfg_path, btn5_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn5_cfg_path, btn5_keys[key])
                        }
                    }
                    ...
                    for (key in btn6_keys) {
                        if (key != btn6_keys.length - 1) {
                            fs.appendFileSync(btn6_cfg_path, btn6_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn6_cfg_path, btn6_keys[key])
                        }
                    }
                    ...
                    for (key in btn7_keys) {
                        if (key != btn7_keys.length - 1) {
                            fs.appendFileSync(btn7_cfg_path, btn7_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn7_cfg_path, btn7_keys[key])
                        }
                    }
                    ...
                    for (key in btn8_keys) {
                        if (key != btn8_keys.length - 1) {
                            fs.appendFileSync(btn8_cfg_path, btn8_keys[key] + "\n")
                        } else {
                            fs.appendFileSync(btn8_cfg_path, btn8_keys[key])
                        }
                    }
